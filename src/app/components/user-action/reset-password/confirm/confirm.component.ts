...
export class ConfirmResetComponent {
  ...
  form: FormGroup;
  token: string | null;
  constructor(private route: ActivatedRoute, private service: ApiUserService,  private router: Router) {
      ...
      this.service.recoverPasswordConfirm(this.form.value).subscribe({
        ...
        error: (error) => {
          ...
          if (error.error.statusCode == 400) {
            this.feedback = 'Hai gà cambiato la password o la password coincide con l\'esisente';
          }
        },
      });
    } else {
      this.feedback = 'token non impostato';
    }
  }
}
